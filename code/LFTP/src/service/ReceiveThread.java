...
public class ReceiveThread implements Runnable {
	private final static int BUFSIZE = 1024 * 2;
	...
	int cwnd = 1024;							// 服务器接受窗口为1Mb，用于流量控制
		...
		try {
			...
			while (true) {
				...
				if(packet.getSeq() == expectedseqnum) {
					...
					// 返回一个正确接受的ACK包
					Packet ackPacket = new Packet(expectedseqnum, -1, true, false, 1, null, fileName);
				...
				// 接受到非期望数据包
					...
					// 返回一个错误接受的ACK包
					Packet ackPacket = new Packet(expectedseqnum-1, -1, true, false, 1, null, fileName);
					byte[] ackBuffer = ByteConverter.objectToBytes(ackPacket);
					DatagramPacket ackdp = new DatagramPacket(ackBuffer, ackBuffer.length, clientInetAddress, clientPort);
					socket.send(ackdp);
